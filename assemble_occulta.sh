...
for((i = 19;i<=35;i=i+2));do mv dn_output_"$i"/transcripts.fa Mocc_dn_k"$i"_transcripts.fa;done 
...
for((i = 19;i<=35;i=i+2));do mv raw_output_"$i"/transcripts.fa Mocc_raw_k"$i"_transcripts.fa;done
...
mv occ_dn_trinity/Trinity.fasta Mocc_dn_tri_transrcipts.fa 
...
Trinity.pl --seqType fq --left left_raw.fq --right right_raw.fq --min_contig_length 200 --CPU 4 --SS_lib_type F --JM 16G --output occ_raw_trinity.fa

mv occ_raw_trinity/Trinity.fasta Mocc_raw_tri_transcripts.fa
